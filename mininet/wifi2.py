...
class module( object ):
    ...
    @classmethod
    def assingIface(self, accessPoints, virtualWlan, stations):
        w = getWlan.virtual()
        #print w
        for ap in accessPoints:
            for i in range(0, ap.nWlans):
                #print virtualWlan
                vwlan = virtualWlan.index("%s0" % str(ap))
                #print vwlan
                #print os.system('iw phy %s list netns %s' % ( emulationEnvironment.totalPhy[vwlan + i], ap.pid ))
                os.system('iw phy %s set netns %s' % ( emulationEnvironment.totalPhy[vwlan + i], ap.pid ))
                #print('ip link set %s name %s-wlan%s up' % (w[vwlan -len(stations) + i], str(ap), i))  
                ap.cmd('ip link set %s name %s-wlan%s up' % (w[vwlan -len(stations) + i], str(ap), i))  
                """
...
class station ( object ):
    ...
    def confirmInfraAssociation(self, node1, node2, wlan):
        ...
        emulationEnvironment.numberOfAssociatedStations(node2) # problem with association
