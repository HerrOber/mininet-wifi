...
from mininet.link2 import Link, Intf, TCIntf, OVSIntf
...
class Node( object ):
    ...
    def setInBandController( self, controllers):
        self.inBandControllers = controllers
...
class Switch( Node ):
    ...
    def __init__( self, name, dpid=None, opts='', listenPort=None, **params):
        ...
        if not self.inNamespace:
            #self.controlIntf = Intf( name, self, port=0 )
...
"""------------------------------------------------------------------------------------------------------"""
"""                                                                                                      """
"""                  CLASSES FOR OVS SWITCHES THAT HAVE THIER OWN NETWORK NAMESPACE                      """
"""                                                                                                      """
"""------------------------------------------------------------------------------------------------------"""
"""------------------------------------------------------------------------------------------------------"""
"""------------------------------------------------------------------------------------------------------"""
...
class NameSpaceOVSSwitch( NameSpaceSwitch ):
    "Open vSwitch switch. Depends on ovs-vsctl."
    ...
    def __init__( self, name, failMode='secure', datapath='kernel',
        ...
        """name: name for switch
           ...
           stp: enable STP (False, requires failMode=standalone)
           batch: enable batch startup (False)"""
        NameSpaceSwitch.__init__( self, name, **params )
        self.failMode = failMode
        self.datapath = datapath
        self.inband = inband
        self.protocols = protocols
    ...
    def start( self, controllers ):
        ...
        if not self.inNamespace:
            raise Exception(
                'switch needs to be in separate namespace' )
        int( self.dpid, 16 )  # DPID must be a hex string
        # Command to add interfaces
...
NameSpaceOVSKernelSwitch = NameSpaceOVSSwitch